    ...
    def get_form_field(self):
        if self.choices:
            ...
            return forms.MultipleChoiceField(
                ...
                # initial=[c[0] for c in self.choices],
                required=False)
...
def get_value(item, choice):
    ...
    for field in fields:
        ...
        if not item:
            break
    return str(value or CSV_EXPORT_EMPTY_VALUE)
...
def csvexport(modeladmin, request, queryset):
    ...
    if 'csvexport' in request.POST and CSV_EXPORT_FORMAT_FORM:
        format_form = CSVFormatForm(request.POST)
    else:
        format_form = CSVFormatForm(dict(
            delimiter=CSV_EXPORT_DELIMITER,
            escapechar=CSV_EXPORT_ESCAPECHAR,
            quotechar=CSV_EXPORT_QUOTECHAR,
            doublequote=CSV_EXPORT_DOUBLEQUOTE,
            quoting=CSV_EXPORT_QUOTING,
            lineterminator=CSV_EXPORT_LINETERMINATOR,
        ))

    # initiate unique-form
    if 'csvexport' in request.POST and CSV_EXPORT_UNIQUE_FORM:
    ...
    n = 0
    while n < CSV_EXPORT_REFERENCE_DEPTH:
        n += 1
    ...
    format_form = format_form if CSV_EXPORT_FORMAT_FORM else None
    unique_form = unique_form if CSV_EXPORT_UNIQUE_FORM else None
